...
export class ZoneProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.zoneSub = this.service.getZoneListByWhEvent().subscribe((res: ZoneProfileEntity[]) => {
      if (res.length > 0) {
        ...
        this.zoneProfileList.forEach((t) => {
          t.isDraw = t.positionX === ' ' ? false : true;
        });
      }
      this.isHidden = this.currentwhId ? true : false;
      this.loadImage().then(() => {
    ...
    this.canvas.addEventListener('click', (e) => {
      ...
      this.modalRef.onHidden.subscribe(() => {
        if (this.modalRef.content.value.isDraw === true) {
          ...
          zone.isDraw = true;
          zone.positionX = String(e.offsetX);
          zone.positionY = String(e.offsetY);
  ...
  drawLocationDetail(zone: ZoneProfileEntity, ctx: any): void {
    ...
    this.ctx.arc(Number(zone.positionX), Number(zone.positionY), 15, 0, Math.PI * 2, false);
    this.ctx.fill();
    this.ctx.arc(Number(zone.positionX), Number(zone.positionY), 15, 0, Math.PI * 2, false);
    ...
    this.ctx.fillText(zone.zoneId, Number(zone.positionX) - 15, Number(zone.positionY) - 20);
  ...
  reset(zone: ZoneProfileEntity): void {
    ...
    this.modalRef.onHidden.subscribe(() => {
      const tZone = this.modalRef.content.value as ZoneProfileEntity;
      this.isReset = true;
      if (tZone) {
        this.zoneProfileList.forEach((temp) => {
          if (temp.zoneId === zone.zoneId) {
